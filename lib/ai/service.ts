...
export const leadSchema = z.object({
  ...
  qualificationScore: z.number().min(1).max(10).describe("Lead quality score 1-10"),
  ...
  potentialValue: z.enum(["High", "Medium", "Low"]).describe("Potential customer value"),
});
...
export async function generateLeads(criteria: {
  ...
  const prompt = `Generate ${count} potential business leads matching these criteria:
...
Create realistic, detailed leads that would be good prospects for B2B services. Each lead should have a unique profile.`;
...
RECOMMENDATIONS: [comma-separated list of next steps]`,
  ...
  const recommendations = lines[3]?.replace("RECOMMENDATIONS: ", "").split(",").map((r) => r.trim()) || [];
 ...
 */
export async function generateOutreachMessage(lead: Lead, context: {
  yourCompany: string;
  yourService: string;
...
RISKS: [comma-separated potential issues or risks, or "none"]`,
  ...
  // Parse response
  const lines = text.split("\n");
  const sentiment = (lines[0]?.toLowerCase().includes("positive")
    ? "positive"
    : lines[0]?.toLowerCase().includes("negative")
      ? "negative"
      : "neutral") as "positive" | "neutral" | "negative";
  const summary = lines[1]?.replace("SUMMARY: ", "") || "";
  const suggestedNextSteps = lines[2]?.replace("NEXT_STEPS: ", "").split(",").map((s) => s.trim()) || [];
  const riskFactors = lines[3]?.replace("RISKS: ", "").split(",").filter((r) => r.trim() !== "none").map((r) => r.trim()) || [];
...
TRENDS: [comma-separated market trends observed]`,
  ...
  return {
    topOpportunities: lines[0]?.replace("TOP_OPPORTUNITIES: ", "").split(",").map((o) => o.trim()) || [],
    commonPatterns: lines[1]?.replace("PATTERNS: ", "").split(",").map((p) => p.trim()) || [],
    recommendations: lines[2]?.replace("RECOMMENDATIONS: ", "").split(",").map((r) => r.trim()) || [],
    marketTrends: lines[3]?.replace("TRENDS: ", "").split(",").map((t) => t.trim()) || [],
  };
}
